...
def random_pose(task_name):
    ...
    if task_name == 'ngrip':
        ...
        gripper1_pos, gripper2_pos, quat = random_rotate(new_mid_point, np.array([x1, y, z1]), np.array([x2, y, z2]),
                                                         z_vec)
    else:
        raise NotImplementedError
...
def main():
    ...
    while i < n_vid:
        ...
        for k in range(n_grips):
            ...
            gripper_rate_limit = [
                (task_params['sample_radius'] * 2 - (task_params['gripper_rate_limits'][0] + 2 * tool_size)) / (
                        2 * task_params['len_per_grip']),
                (task_params['sample_radius'] * 2 - (task_params['gripper_rate_limits'][1] + 2 * tool_size)) / (
                        2 * task_params['len_per_grip'])]
            ...
            while counter < task_params["len_per_grip"]:
                ...
                actions.append(np.concatenate(
                    [prim1_action / 0.02, zero_pad, -prim1_action / 0.02, zero_pad]))  # pose for both grippers
                counter += 1
            counter = 0
            ...
            for idx, act in enumerate(tqdm(actions, total=actions.shape[0])):
                env.step(act)
                obs = get_obs(env, 300)  # x: [304, 3], v: [304, 3]
                x = obs[0][:300]
                ...
                os.system('mkdir -p ' + f"{rollout_dir}/{i:03d}/rgb")
                os.system('mkdir -p ' + f"{rollout_dir}/{i:03d}/depth")

                for num_cam in range(4):
                    cv2.imwrite(f"{rollout_dir}/{i:03d}/rgb/{num_cam}_{true_idx:03d}.png", rgb[num_cam][..., ::-1])

                    depth_tmp = depth[num_cam][..., ::-1]
                    depth_tmp[depth_tmp > 5] = 0
                    depth_tmp = depth_tmp / 5.0000
                    # cv2.imshow('imgOri',depth_tmp)
                    # cv2.waitKey(0)
                    depth_tmp = depth_tmp * 255
                    cv2.imwrite(f"{rollout_dir}/{i:03d}/depth/{num_cam}_{true_idx:03d}.png", depth_tmp)
        ...
        os.system(
            f'ffmpeg -y -i {rollout_dir}/{i:03d}/rgb/0_%03d.png -c:v libx264 -vf fps=25 -pix_fmt yuv420p {rollout_dir}/{i:03d}/vid{i:03d}_rgb.mp4')
        os.system(
            f'ffmpeg -y -i {rollout_dir}/{i:03d}/depth/0_%03d.png -c:v libx264 -vf fps=25 -pix_fmt yuv420p {rollout_dir}/{i:03d}/vid{i:03d}_depth.mp4')
        i += 1
